...
unsigned
count_using_iterator( ys_bitset_t *bs )
{
	YASENS BitSetIterator iter(bs);
	unsigned n = 0;
	while (iter.getNext() != -1)
		n++;
	return n;
}
...
int testbitset(void)
{
	...
	assert(count_using_iterator(bs1) == 2);
	...
	assert(count_using_iterator(bs2) == 2);
