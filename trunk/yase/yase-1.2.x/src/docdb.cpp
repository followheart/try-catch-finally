	...
	ys_docnum_t *dn)
{
	...
	offset = docnum * (sizeof offset + sizeof type + sizeof wt + sizeof maxdtf);
	...
	ys_file_write(&maxdtf, 1, sizeof maxdtf, db->yasedocptrs);
...
ys_dbadddocweight(ys_docdb_t *db, ys_docnum_t docnum, float wt)
{
	...
	pos = docnum * (sizeof pos + sizeof type + sizeof f + sizeof maxdtf);
 ...
 */
int
ys_dbadddocmaxdtf(ys_docdb_t *db, ys_docnum_t docnum, ys_doccnt_t maxdtf)
{
	ys_filepos_t pos;
	char type;
	float f;
	...
	pos = docnum * (sizeof pos + sizeof type + sizeof f + sizeof maxdtf);
	pos += (sizeof type + sizeof pos + sizeof f);
	ys_file_setpos(db->yasedocptrs, &pos);
	ys_file_write(&maxdtf, 1, sizeof maxdtf, db->yasedocptrs);
	if (ys_file_error(db->yasedocptrs)) {
		fprintf(stderr, "Unable to write to yase.docptrs\n");
 ...
 */
int
ys_dbgetdocmaxdtf(ys_docdb_t *db, ys_docnum_t docnum, ys_doccnt_t *maxdtf)
{
	float f;
	ys_filepos_t pos;
	char type;
	...
	pos += (sizeof type + sizeof pos + sizeof f);
	ys_file_setpos(db->yasedocptrs, &pos);
	ys_file_read(&temp, 1, sizeof temp, db->yasedocptrs);
	if (ys_file_error(db->yasedocptrs)) {
		fprintf(stderr, "Unable to read from yase.docptrs\n");
		return -1;
	}
...
int
ys_dbgetdocwtdtf(ys_docdb_t *db, ys_docnum_t docnum, float *wt, ys_doccnt_t *maxdtf)
{
	float f;
	ys_filepos_t pos;
	char type;
	...
	ys_docdata_t *doc )
{
	...
	pos = docnum * (sizeof pos + sizeof type + sizeof wt + sizeof maxdtf);
	ys_file_setpos(db->yasedocptrs, &pos);
	...
	ys_file_read(&maxdtf, 1, sizeof maxdtf, db->yasedocptrs);
	if (ys_file_error(db->yasedocptrs)) {
		fprintf(stderr, "Unable to read from yase.docptrs\n");
		return -1;
	}
