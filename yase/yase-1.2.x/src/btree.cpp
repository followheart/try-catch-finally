 ...
 *             these functions.
 * DM 03-12-12 Implemented B+Tree 
 * DM 03-12-02 Found a bug in ys_block_set_curkey() and fixed it.
 * DM 07-02-03 Added ys_btree_find() and ys_btree_verify().
 */
/***
...
{
	//enum { BYTESIZE = 8 };
	//int x = YS_DIVIDE_AND_ROUNDUP(ys_most_significant_bit(n)+1,BYTESIZE);
	int y = 0;
	if (n != 0) {
		if (n <= 0xFFu) y = 1;
		else if (n <= 0xFFFFu) y = 2;
		else if (n <= 0xFFFFFFu) y = 3;
		else y = 4;
	}
	//assert(x == y);
	//return x;
...
static inline ys_uchar_t *
ys_write_entry_prologue(ys_uchar_t *cp, ys_len_t pl, ys_len_t kl,
		       ys_len_t bs, ys_len_t vs, ys_len_t cs,
		       ys_doccnt_t cnt)
{
	*cp++ = (ys_uchar_t)pl;
	*cp++ = (ys_uchar_t)kl;
...
static inline ys_uchar_t *
ys_read_entry_prologue(ys_uchar_t *cp, ys_len_t& pl, ys_len_t& kl,
		       ys_len_t& bs, ys_len_t& vs, ys_len_t& cs,
		       ys_doccnt_t& cnt)
{
	pl = *cp++;
...
static inline ys_uchar_t *
ys_write_blocknum(ys_uchar_t *cp, ys_blocknum_t n, ys_len_t len)
{
	ys_len_t i;
	for (i = 0; i < len; i++, cp++) 
		*cp = (ys_uchar_t) (0xff&(n>>(8*i)));
	return cp;
}

static inline ys_uchar_t *
ys_read_blocknum(ys_uchar_t *cp, ys_blocknum_t& n, ys_len_t len)
{
	ys_len_t i;
	for (n = 0, i = 0; i < len; i++, cp++)
		n |= ((*cp)<<(8*i));
	return cp;
}

static inline ys_uchar_t *
ys_write_filepos(ys_uchar_t *cp, ys_filepos_t n, ys_len_t len)
{
	ys_len_t i;
	for (i = 0; i < len; i++, cp++) 
		*cp = (ys_uchar_t) (0xff&(n>>(8*i)));
...
static inline ys_uchar_t *
ys_read_filepos(ys_uchar_t *cp, ys_filepos_t& n, ys_len_t len)
{
	ys_len_t i;
	for (n = 0, i = 0; i < len; i++, cp++)
		n |= ((*cp)<<(8*i));
...
static inline ys_uchar_t *
ys_write_doccnt(ys_uchar_t *cp, ys_doccnt_t n, ys_len_t len)
{
	ys_len_t i;
	for (i = 0; i < len; i++, cp++) 
		*cp = (ys_uchar_t) (0xff&(n>>(8*i)));
...
static inline ys_uchar_t *
ys_read_doccnt(ys_uchar_t *cp, ys_doccnt_t& n, ys_len_t len)
{
	ys_len_t i;
	for (n = 0, i = 0; i < len; i++, cp++)
		n |= ((*cp)<<(8*i));
...
static inline ys_uchar_t *
ys_write_key(ys_uchar_t *cp, const ys_uchar_t *key, ys_len_t prefix_len, ys_len_t key_len)
{
	key += (1+prefix_len);
...
ys_block_calc_offsets(ys_block_t *block)
{
	...
	for (k = 0; k < block->bd.keycount; k++) {
		...
		cp = ys_read_entry_prologue(cp, prefix_len, key_len, bnsize, vlsize,
			ctsize, ct);
...
ys_block_read_curkey(ys_block_t *block)
{
	...
	key = cp;
	cp += key_len;
	cp = ys_read_blocknum(cp, bn, bnsize);
	cp = ys_read_filepos(cp, vl, vlsize);
	if (ctsize > 0)
		ys_read_doccnt(cp, ct, ctsize);
...
ys_block_write_entry(ys_uchar_t *cp, ys_uchar_t *key, ys_len_t prefix_len,
	...
	ys_filepos_t vl, ys_len_t vlsize, ys_doccnt_t ct, ys_len_t ctsize)
{
	cp = ys_write_entry_prologue(cp, prefix_len, key_len, bnsize, vlsize, 
		ctsize, ct);
	cp = ys_write_key(cp, key, prefix_len, key_len);
	cp = ys_write_blocknum(cp, bn, bnsize);
	cp = ys_write_filepos(cp, vl, vlsize);
	if (ct > 0x0f)
		cp = ys_write_doccnt(cp, ct, ctsize);
	return cp;
}
...
int
ys_bplustree_create( const char *treename, const char *inputname )
{
	ys_file_t *fp = NULL;
	ys_file_t *tempfp = NULL;
	char tempfile[1024];
	...
	while ( fp != NULL ) {
		...
		while ( (rec = ys_get_record(fp, leaflevel)) != NULL ) {
			...
			if (status != 0) {
				...
				block->dirty = BOOL_TRUE;
				nblock = ys_block_new( file, leaflevel );
				if (leaflevel) {
					block->bd.lowernode = nblock->blocknum;
				}
				assert(block->ref == 1);
				ys_blockfile_put( file, block );
				// block = ys_block_new( file, leaflevel );
				...
				if (leaflevel) {
					status = block_addentry( block, e.key, e.bn, 
						e.vl, e.doccnt);
					if (status != 0) {
						fprintf(stderr, "Unexpected error at file %s line %d\n",
							__FILE__, __LINE__);
						exit(1);
					}
					block->bd.lowernode = 0;
...
ys_btree_open_mode( const char *treename, const char *mode )
{
	...
	tree = (ys_btree_t *) calloc(1, sizeof(ys_btree_t));
	if (tree == NULL) {
		perror("calloc");
 ...
 */
static ys_bool_t
ys_block_search_bplus(ys_block_t *block, ys_uchar_t *key, ys_filepos_t *valueptr,
	ys_doccnt_t *doccntptr, ys_blocknum_t *blockptr)
{
	ys_keycnt_t i;
	*blockptr = block->bd.lowernode;
	for (i = 1; i <= block->bd.keycount+1; i++) {
		...
		if (result == 0 && ys_block_isleaf(block)) {
 ...
 */
int
ys_bplustree_iterate( ys_btree_t *tree, ys_uchar_t *key, ys_pfn_btree_iterator_t *pfunc, void *arg )
{
	ys_block_t *block;
 ...
 */
bool
ys_btree_find( ys_btree_t *tree, ys_uchar_t *key, ys_filepos_t *value, ys_doccnt_t *doccnt )
{
	ys_block_t *block;
	ys_blocknum_t blockptr = 0;
	ys_bool_t found = BOOL_FALSE;
	...
	block = ys_block_getblock( tree->file, tree->root );
	while (!ys_block_isleaf(block)) {
		found = ys_block_search(block, k, value,
			doccnt, &blockptr);
		if (found) 
			goto done;
		assert(blockptr != 0);
	...
	ys_doccnt_t ct)
{
	...
	vl2 = block->curvalue;
	ct2 = block->curdoccnt;
	cp = ys_read_entry_prologue(cp, prefix_len1, key_len1, bnsize2, vlsize2,
		ctsize2, ctdummy);
...
int
ys_btree_verify( const char *treename, const char *inputname )
{
	ys_file_t *fp = NULL;
	ys_bool_t leaflevel = BOOL_TRUE;
	...
	if ( fp != NULL && tree != NULL ) {
		...
		while ( (rec = ys_get_record(fp, leaflevel)) != NULL ) {
			...
			if ( !ys_btree_find( tree, e.key, &e.vl, &e.doccnt ) ) {
				fprintf(stderr, "Error: failed to verify key %.*s (key not found)\n",
					e.key[0], e.key+1);
				rc = -1;
				break;
			}

			if ( e.vl != rec->posting_offset ||
				e.doccnt != rec->doccnt ) {
				fprintf(stderr, "Error: failed to verify key %.*s (mismatched value/doccnt)\n",
					e.key[0], e.key+1);
				rc = -1;
				break;
			}
